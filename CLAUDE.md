...
- **Claude Code config backup/restore**: `claude-config.sh` backs up `settings.json`, `commands/`, `plugins/` (skills/marketplaces), and MCP server settings to `/mnt/c/Users/techjuice/Documents/dev/.claude-config/` for sharing across WSL instances (same pattern as SSH key sharing). MCP restore uses `jq` to merge only the `mcpServers` key, preserving existing auth data. Skills are restored by copying the entire `plugins/` directory.
